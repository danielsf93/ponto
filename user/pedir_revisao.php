...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ponto_id'], $_POST['nova_data_hora'], $_POST['justificativa'])) {
        ...
        // Converter o valor do campo datetime-local (formato "YYYY-MM-DDTHH:MM") para "YYYY-MM-DD HH:MM:SS"
        if (strpos($nova_data_hora, 'T') !== false) {
            $nova_data_hora = str_replace('T', ' ', $nova_data_hora) . ':00';
        }
        ...
        if (!$ponto) {
            ...
            // Insere o pedido de revisão na tabela de pedidos_revisao
  ...
  <div class="container">
    ...
    <?php if (isset($pontoParaRevisao)): ?>
      ...
      <form method="POST">
         ...
         <label for="nova_data_hora">Nova Data e Hora:</label>
         <input type="datetime-local" name="nova_data_hora" id="nova_data_hora" required>
         <br>
      ...
      <br>
      <a href="/ponto/user/pedir_revisao.php">Cancelar</a>
    <?php else: ?>
      <h3>Meus Pontos</h3>
        ...
        <table border="1">
          ...
          <?php foreach($pontos as $ponto): ?>
            <tr>
              ...
              <td>
                <a href="/ponto/user/pedir_revisao.php?ponto_id=<?= htmlspecialchars($ponto['id']) ?>">Pedir Revisão</a>
              </td>
            </tr>
          <?php endforeach; ?>
        </table>
